...
fn draw_black_frame(pix: &mut agrega::Pixfmt<agrega::Rgb8>) {
    ...
    let h = pix.height();
    for i in 0..h {
        pix.set((0, i), agrega::Rgb8::black()); // Left Side
        pix.set((w - 1, i), agrega::Rgb8::black()); // Right Side
    }
    for i in 0..w {
        pix.set((i, 0), agrega::Rgb8::black()); // Top Side
        pix.set((i, h - 1), agrega::Rgb8::black()); // Bottom Side
    }
}
...
fn t01_rendering_buffer() {
    ...
    for i in 0..pix.height() / 2 {
        ...
        pix.set((i, i), agrega::Rgb8::new(127, 200, 98));
    }

    pix.to_file("tests/std/tmp/agg_test_01.png").unwrap();
    assert_eq!(
        agrega::ppm::img_diff("tests/std/tmp/agg_test_01.png", "tests/images/agg_test_01.png")
            .unwrap(),
        true
    );
}
